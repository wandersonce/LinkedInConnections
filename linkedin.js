...
const linkedin = {
  ...
  login: async (username, password) => {
    ...
    let loginButton = await linkedin.page.$x(
    ...
    let asideMsg = await linkedin.page.waitFor('aside > div');

    if (asideMsg) {
      // If it is opened it will close
      let headerAside = await linkedin.page.waitFor('aside > div > header');
  ...
  connectionProcess: async () => {
    ...
    for (let i = 0; i < 3; i++) {
      //Here change how many time it will scrolldown, it will change the total of the users to connect.
    ...
    let person = await linkedin.page.$$(
